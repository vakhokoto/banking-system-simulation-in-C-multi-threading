             ...
             AccountAmount initialAmount)
{
  ...
  pthread_mutexattr_init(&(account -> attr));
  pthread_mutexattr_settype(&(account -> attr), PTHREAD_MUTEX_RECURSIVE_NP);
  pthread_mutex_init(&(account -> lock), &(account -> attr));
...
Account_Adjust(Bank *bank, Account *account, AccountAmount amount,
               int updateBranch)
{
  BranchID branchID = AccountNum_GetBranchID(account -> accountNumber);
