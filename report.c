...
Report_Init(Bank *bank, AccountAmount reportAmount, int maxNumWorkers)
{
  ...
  bank -> maxNumWorkers = maxNumWorkers;
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  pthread_mutex_lock(&(bank -> lock));
  bank -> numWorkersEntered++;

  if (bank -> numWorkersEntered == bank -> maxNumWorkers){
    ...
    */
    ...
    bank -> numWorkersEntered = 0;
    pthread_cond_broadcast(&(bank -> cond));
    pthread_mutex_unlock(&(bank -> lock));

  } else {
    pthread_cond_wait(&(bank -> cond), &(bank -> lock));
    pthread_mutex_unlock(&(bank -> lock));
  }
  return err;
}
